...
namespace hexeract
{
	namespace content
	{
		...
		path Registry::findGenericFirst(const std::string& name)
		{
			path result;
		...
		bool Registry::findGenericFirst(const std::string& name, path& result)
		{
		...
		std::vector<path> Registry::findGenericAll(const std::string& name)
		{
		...
		bool Registry::findObject(std::type_index tid, const std::string& name, path& result)
		{
		...
		std::unique_ptr<NamedObject> Registry::getFromDisk(const object_type& type, const std::string& name)
		{
			return getFromDiskInt<NamedObject>(type, name);
		}

		std::unique_ptr<NamedObject> Registry::getFromJson(const object_type& type, const nlohmann::json& json)
		{
			return getFromJsonInt<NamedObject>(type, json);
		}

		bool Registry::findShaderFile(const path& p, path& result) const
		{
			path tmp;
			if (p.is_relative()) {
		...
		void Registry::scanContentFolder(const path& p)
		{
		...
		void Registry::addFile(const path& p)
		{
			std::string name;
		...
		std::string Registry::getAnonName()
		{
			auto fmt = boost::format("unnamed_%1$4i") % (m_anonCounter++);
			return fmt.str();
		}

		std::string Registry::getObjectName(const nlohmann::json& json)
		{
			std::string result;
		...
		SCRIPTING_REGISTER_STATIC_CLASS(Registry);

		SCRIPTING_DEFINE_METHOD(Registry, get)
		{
