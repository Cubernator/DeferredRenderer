...
namespace hexeract
{
	namespace graphics
	{
		shader_preprocessor::shader_preprocessor(const path& fname, unsigned int srcId) : m_filename(fname),
			m_shaderType(Shader::type_undefined), m_error(false), m_srcId(srcId), m_lg("Shader preprocessor")
		{
		...
		shader_preprocessor::shader_preprocessor(std::istream& stream, unsigned int srcId) :
			m_shaderType(Shader::type_undefined), m_error(false), m_srcId(srcId), m_lg("Shader preprocessor")
		{
			process(stream);
		}
		...
		void shader_preprocessor::include(const path& file)
		{
			...
			if (file.is_absolute()) {
				...
				found = exists(filepath);
				if (!found) {
					found = instance<content::Registry>()->findShaderFile(file, filepath);
				}
			}
