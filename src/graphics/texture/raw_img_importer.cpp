...
raw_img_importer::raw_img_importer(const path& file, const nlohmann::json& options)
	: m_width(0), m_height(0), m_pixelTraits(get_pixel_traits<pixel::rgb8>()), m_status(false)
{
	int imgSize;
	...
	if (texFile) {
		...
		auto stride = m_pixelTraits.pixelSize;
...
keyword_helper<pixel_traits> raw_img_importer::s_formats({
